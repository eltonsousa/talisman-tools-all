...
if [[ "$SIM_NAO" =~ ^[Ss]$ ]]; then
    ...
    sudo -u $USER_TALISMAN screen -dmS db /bin/bash -c "cd $DIR_DB && ./db_server"
    sleep 5
    echo -e " ${VERDE_B}PRONTO!${SEM_COR}"
    
    # --- [2] INICIAR LOGIN SERVER ---
    echo -ne "[2/3] Iniciando Login Server..."
    sudo -u $USER_TALISMAN screen -dmS login /bin/bash -c "cd $DIR_LOGIN && ./login_server"
    ...
    sudo -u $USER_TALISMAN screen -dmS game /bin/bash -c "cd $DIR_GAME && ./game_server"
    echo -e " ${VERDE_B}PRONTO!${SEM_COR}"
